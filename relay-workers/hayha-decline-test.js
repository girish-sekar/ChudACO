  ...
  async fetch(request, env) {
    try {
      ...
      if (isTest(body)) {
        ...
        await Promise.all([
          post(env.DISCORD_WEBHOOK_DECLINED, testPayload("ChudACO Declines", true, mentionUserId)),
          postToChudaco(env, s)
        ]);
        return ok("test");
      }
      ...
      const mentionUserId = await resolveMentionId(env, s.profile);
      console.log("decline mentionUserId", mentionUserId || "none", "profile", s.profile);

      await Promise.all([
        post(env.DISCORD_WEBHOOK_DECLINED, declinePayload("ChudACO Declines", s, reason, kind, mentionUserId)),
        postToChudaco(env, s)
      ]);
      return ok("forwarded");
    } catch (e) {
...
function declinePayload(name, s, reason, kind, mentionUserId) {
  const embed = {

export default {
  async fetch(request, env) {
    try {
      ...
      if (isTest(body)) {
        await post(env.DISCORD_WEBHOOK_DECLINED, testPayload("ChudACO Declines", true));
        return ok("test");
      }
      ...
      await post(env.DISCORD_WEBHOOK_DECLINED, declinePayload("ChudACO Declines", s, reason, kind));
      return ok("forwarded");
    } catch (e) {

export default {
  async fetch(request, env) {
    try {
      ...
      if (isTest(body)) {
        await post(env.DISCORD_VALOR_WEBHOOK_SUCCESS, testPayload("ChudACO Success", false));
        return ok("test");
      }

      if (!isValorSuccess(body)) return ok("not success");

      const s = sanitize(body);
      await post(env.DISCORD_VALOR_WEBHOOK_SUCCESS, successPayload("ChudACO Success", s));
      return ok("forwarded");
    } catch (e) {
...
function isValorSuccess(b) { return /checked out|successful checkout|checkout success|\bsuccess\b/.test(blob(b)); }
